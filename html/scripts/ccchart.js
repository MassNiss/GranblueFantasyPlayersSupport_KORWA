...
(function(window) {
    return {
            ...
            drowHanrei: function () {
                ...
                var shdw = this.shadows.hanrei||this.shadows.all||['#222', 5, 5, 5];
                ...
                for(var i = 0; i < this.hanreiNames.length; i++){
                    this.ctx.beginPath();
                    this.drawShadow(shdw[0],shdw[1],shdw[2],shdw[3]);
            ...
            drawXLine: function () {
                ...
                for (var i = 0; i < op.length; i++) {
                    ...
                    var shdw = this.shadows.xline||this.shadows.all||['#444', 7, 7, 5];
                    //line
                    this.ctx.beginPath();
                    this.ctx.lineWidth = lineWidth;
            ...
            _drawLine: function (op, func) {
                ...
                var colorSet = op.colorSet || this.colorSet;
                var shdw = this.shadows.line||this.shadows.all||['#222', 5, 5, 5];
                for(var k = 0; k < this.dataRowLen; k++ ){
                    var x = this.chartLeft;
                    x += this.xGap/2; //オフセット
                    this.ctx.beginPath();
            ...
            _drawStackedArea: function (op, func) {
                ...
                var lineWidth = op.lineWidth || this.lineWidth;
                var colorSet = op.colorSet || this.colorSet;
                var shdw = this.shadows.stackedarea||this.shadows.all||['#222', 5, 5, 5];
                var wkData = [];
                var wkMaxYs = [];
                for(var k = 0; k < this.dataRowLen; k++ ){
                    wkData[k] = [];
            ...
            drawBar: function () {
                var that = this;
                var shdw = this.shadows.bar||this.shadows.all||['#222', 5, 5, 5];
            ...
            drawStackedBar: function () {
                ...
                var that = this;
                var x = that.barPadding + that.chartLeft;
                var shdw = this.shadows.stacked||this.shadows.all||['#222', 5, -5, 5];
            ...
            drawbeziLine: function () {
                this.ctx.save();
                var recx=0,recy=0;
                var shdw = this.shadows.bezi||this.shadows.all||['#222', 5, 5, 5];
                for(var k = 0; k < this.dataRowLen; k++ ){
                    var x = this.chartLeft;
                    x += this.xGap/2; //オフセット
                    this.ctx.beginPath();
            ...
            drawbeziLine2: function () {
                var shdw = this.shadows.bezi2||this.shadows.all||['#222', 5, 5, 5];
                //ベジェグラフを描く
                this.ctx.save();
                for(var k = 0; k < this.dataRowLen; k++ ){
                    var x = this.chartLeft;
            ...
            drawScatter: function () {
                //var shdw = this.shadows.scatter||this.shadows.all||['#222', 5, 5, 5];
            ...
            drawPie: function(){
                this.ctx.save();
                var that = this;
                var shdw = this.shadows.pie||this.shadows.all||['#444', 3, 3, 3];
            ...
            ws: function(url, cb){
              ...
              function _w(url, id){
                  ...
                  w.uid = uid;
                  w.on('open', function () { console.log('ws opened: ' + url + ' at ' + id); });
                  w.on('close' , function () { 
            ...
            _moveByWS: function (cvs, chartdata, url, op, delay, step) {
              var mdata = [];
              mdata[this.id] = [];
              var that = this;
              var type = 'movebyws';
              ...
              var wstid = []; // タイマーID
              var wsq = []; // データを仮に入れる暫定配列
              var timerDrawing = []; // 描画中ですフラグ
              var delay = delay || 1000;
              var newData = null;
              var recData = null;
              var movebywsInitFlg = 0;
              var id = cvs.id;

              function mkWS() {
                return that.ws('ws://' + url);
              }
              if(!this._ws) {
                this._ws = [];
                this._ws[id] = mkWS(); // ws無ければ接続
              }
              else if(!this._ws[id]) {
                this._ws[id] = mkWS();
              }
              else {
                if(this._ws[id].readyState !== this._ws[id].OPEN) 
                  this._ws[id] = mkWS(); // 切れたら再接続
              }
              this._ws[id].cchart = this;
              this._ws[id].onopen = function () { console.log('ws opened: ' + url); }
              this._ws[id].onclose = function () { console.log('ws closed: ' + url); }
              this._ws[id].onmessage = function (e) {

                try {
                  var newid = '' + (new Date()).getTime() + (Math.random());
                  if(that.drawing) return;           // グラフ描画中なら着信データを捨てる
                  if(timerDrawing[newid]) return;    // タイマ描画中なら着信データを捨てる
                  var data = JSON.parse(e.data);
                  // if(wsq === [data]) return;         
                          // 着信データが項目前回と同じなら捨てる
                          //これは、一般的には、2012,2013,2014...といった列項目がある場合は有効だが
                          //データのみの場合は、連続する同一データも有りうるので単純な比較ではダメ
                  // wsq.unshift(data); // (a) 暫定配列の先頭へデータをいれる 遅延処理の場合
                  wsq=[data] ;          // 暫定配列へデータをいれる 最新で上書き

                  if(movebywsInitFlg === 0) {//初回は即実行
                    var dataid = getDataId([data],  op, cvs.id);
                    drawIt(e, id, config, dataid[0], cvs, dataid[1], [data]);
                  }
                  //描画速度調整用タイマー開始
                  // (サーバープッシュ過剰時のクライアント負荷も回避する)
                  clearTimeout(wstid[newid]);
                  timerDrawing[newid] = true;              // タイマ描画開始のフラグ
                  wstid[newid] = setTimeout(function () {
                    var dataid = getDataId([data],  op, cvs.id);
                    drawIt(e, id, config, dataid[0], cvs, dataid[1], [data]);
                  }, delay)
              ...
              function getDataId(wsq,  op, id){
                //wsq = wsq.slice(0, 1);  // (a)に対応 先頭の最新データのみ受け取る。残りは捨てる
                   //(a)ではサーバーからのDosに負ける
                return mkdata(wsq, op, id); // type別にデータ追加と削除
              }

              function drawIt(e, id, config, newData, cvs, newid, wsq){
                var it = that;//注意：このthatはすでにcurrentではない。
                //チャートデータ作る
                var chartdata = {
                  "config": config,
                  "data": newData
                ...
                if(it.ops[cvs.id].id)
                  if(op.onwsdraw){
                      op.onwsdraw.apply(it,[e, id, wsq[0]]);
                        // if(op.onwsdraw)op.onwsdraw(e, wsq[0]);
                  }
                 
                //描画
                it.init(cvs, chartdata,
                  function () {
                    timerDrawing[newid] = false; // タイマ描画完了
                    clearTimeout(wstid[newid]);  // ロック解除
                  }
                );
              }

              function mkdata(datas, op, id) {

                if(type === 'movebywsfull') {
                  return [datas[0], id];
                }
                else if(type === 'movebyws') {
                  var data = datas[0];
                  if(movebywsInitFlg === 0) {
                    //受け取ったデータ数に合わせた2次元配列作成
                    ...
                    if(op.side === 'right') {
                      (data).forEach(function (x, i) {
                        mdata[id][i].push(x); // 後ろへ追記
                        // colLen超えたら先頭を削除
                        if(mdata[id][i].length > op.colLen) mdata[id][i].shift();
                      });
                    }
                    else {
                      (data).forEach(function (x, i) {
                        mdata[id][i].unshift(x); // 頭へ追記
                        // colLen超えたら末尾を削除
                        if(mdata[id][i].length > op.colLen) mdata[id][i].pop();
                      });
                    }
                    //項目名を頭につける
                    var wk = JSON.parse(JSON.stringify(chartdata.data));
                    for(var i = 0; i < chartdata.data.length; i++) {
                      wk[i] = chartdata.data[i].concat(mdata[id][i])
                    }
                  }
                  
                 // console.log('mkdata: ', wk, id );
            ...
            //WebSocket で差分のみ送信
            moveByWS: function(cvs, chartdata, url, op, delay){
              if((!op || typeof op !== 'object') || this.util.isArray(op))var op={};
              op.colLen = (op.colLen)?op.colLen:18;
              op.side   = (op.side)?op.side:'right';
              this._moveByWS (cvs, chartdata, url, op, delay);
              return this;
            },
            //調整中：
